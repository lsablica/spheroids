...
arma::mat Moebius_S(const arma::mat &X, arma::vec mu, double rho){
...
// [[Rcpp::export]]
List EM(const arma::mat &data_arr, int K, String E_type, String M_type,  double minalpha = 0.05, int maxiter = 100, double reltol = 1e-6){
  arma::mat data = arma::normalise(data_arr, 2, 1); // normalize in case
  ...
  return  List::create(beta_matrix, rho_vec.t(), mu_matrix, pi_vector, log_lik, i);
} 